...
- `frontend/public/icons/crias-ai-mark.svg` for the owner-supplied green vector artwork;
...
Use the same owner-supplied green composition for the Dock, Finder, notifications, favicon, PWA, and documents. The macOS asset adds only the platform corner mask; it does not redraw or recolor the artwork. Do not reconstruct the dot pattern, stretch it, add per-dot shadows, or replace semantic warning/error colors with Jade.
