...
No item in the table below becomes `READY` merely because it compiles. The owner app was packaged and installed on 2026-08-25. The 3090 inference process was deliberately left running; gateway and cache changes that need a service restart remain dependencies rather than inferred passes.
...
| General, updates, controllers | CODED — UNVALIDATED | Owner-release update protection and two-step confirmation exist through `f88f9a74`; deploy output is redacted before IPC in `33bf8864`; manual deploy is demoted to Advanced. | The batch is installed; a real fork update replacement attempt has not been accepted. | Verify the owner channel cannot be replaced by an upstream update while tailnet publishing is active. |
| System | CODED — UNVALIDATED | Capability-aware states exist in `fa3a19c8`, `ee412bfc`, and `47c5dbc2`; controller state is scoped in `fbfc9715`; unsupported data is not fabricated. The installed desktop now synchronizes the authoritative controller URL before probing capabilities, and the legacy plain-text `/logs` response is no longer mistaken for the structured logs contract. | The installed app boots against the saved Tailscale controller instead of the stale browser-local loopback default. | Full capable-controller acceptance waits for the delayed gateway restart. Unsupported and limited states must remain explicit rather than fabricated. |
...
| Status telemetry | CODED — UNVALIDATED; DEPENDENCY on stack restart | Trends originate in `de92de55`; `a51fb75a` isolates controller/process epochs and `856021cc` samples only observed payloads. `bbfef796`, stack commit `aad1e95`, and `af047761` bind completed-request performance to the resident physical model and process start epoch, including PID reuse. GPU utilization, VRAM, board power, and temperature are live samples; decode, prefill, and TTFT are timestamped observations from the latest completed request; active requests come from the live engine slot state. | The installed Status surface shows official model identity, metric/range selectors, GPU inventory, VRAM, temperature, power, and honest no-sample states. | The deployed gateway code is not active until its delayed restart, so decode/prefill/TTFT history and log-stream acceptance are still pending. Queue depth remains unavailable. Whole-PC power requires a real wall meter or UPS source; estimates are refused. |
...
| Server health and logs | CODED — UNVALIDATED; DEPENDENCY on stack restart | The 3090 stack branch has bounded, allowlisted, redacted journal endpoints in `1412312` through `8f5ce9a`; `c038d031` isolates frontend sessions and `33bf8864` protects deploy output. | Stack code at `aad1e95` is deployed on the host with a rollback backup, but the running services intentionally remain on the previous process image. The installed client correctly treats the current plain-text `/logs` endpoint as unsupported. | Restart the gateway only after the owner releases the active inference process, then validate every stream and redaction boundary. |
...
- The owner released the desktop app for packaging on 2026-08-25. The final app, ZIP, and `Local Studio-2.1.0-arm64.dmg` were built; the previous install was archived for rollback; the new app was installed and booted successfully.
...
- `npm run check` passed after the final source changes. The 3090 restart-dependent items above remain explicitly outside this desktop release acceptance.
- Commit, push, and the repository release workflow follow after this evidence is recorded; no dependency is rounded up to `READY` merely because the desktop package is installed.
