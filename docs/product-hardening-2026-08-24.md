...
This is one cross-cutting triad applied to every area, not three independent implementations of the same feature. Its evidence is static until the release gate is opened. The final branch-wide review found three confirmed defects: stale Usage scope in the embedded controller, nondeterministic multi-image actions, and one missing project taking down global search. All three were reproduced, corrected, and rechecked before packaging; that verdict does not replace packaged or live acceptance.
...
- The embedded controller validates and echoes requested Usage period, model, and timezone on success, cache hit, empty-store, and fallback paths. Its flat legacy aggregates remain lifetime by contract, and it leaves scoped tokens, energy, and efficiency absent because it does not record the richer buckets required to calculate them honestly.
