...
MEASURED / PROVEN — root's installed final10 stable session retained an uncensored model selection whose custom physical-model label hid the restricted behavior profile. Sending a turn produced the correct backend HTTP 400 admission rejection with an actionable explanation, but the composer cleared and the visible chat showed only the user message. Source tracing found that the submit handler already stored the rejection in the conversation's `error` field; ChatPane never rendered that field. A legacy restricted default also made the physical group's existing default pin inert, obstructing an explicit same-model repair to Standard.
