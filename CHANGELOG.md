...
Jade Horizon accent across the macOS package, Finder and Dock icon, desktop UI,
PWA manifest, Home Screen icon, favicon, pairing flow, and brand documentation.
Compatibility identifiers, API paths, environment variables, and the existing
data directory remain stable so the rebrand does not split sessions or secrets.

- The phone control opens a true centered modal with a two-minute, one-use QR
  pairing ticket. The durable token never enters renderer state or the DOM, the
  composer is inert behind the modal, and the tailnet URL plus masked recovery
  token remain available without exposing the secret.
- Global search now includes bounded transcript-content matches across projects
  and archived conversations alongside app destinations. Selecting a result
  opens the owning project and session. A missing project or unreadable
  transcript is isolated instead of taking every valid search result down.
- Generated image tools render an aspect-ratio placeholder while work is in
  flight, then an interactive image with lightbox, copy, download, Approve,
  Reject, and Regenerate. Multi-image results act on the selected image and
  surface failures rather than acknowledging an action that was never sent.
  The composer also accepts and preserves queued attachments during a turn.
- Settings now describe the actual owner topology. Unsupported system facts,
  raw capability 404s, obsolete phone pairing, and unreachable recipe/model
  controls no longer look like broken primary workflows. Setup treats optional
  services as optional, logs are bounded and redacted, and official physical
  model names replace routing aliases in owner-facing UI.
- Status offers persistent metric and range selection for measured GPU,
  request, throughput, TTFT, temperature, VRAM, and board-power observations.
  Usage keeps period, model, and timezone aligned through cache and fallback
  paths; unmeasured whole-host power and rich accounting remain absent rather
  than being estimated.
- Durable Runs have Current, History, and Archived views with safe restore and
  terminal-only deletion. Logical agents share the single inference boundary
  honestly, while isolated rollout sessions remain hidden from chat navigation.
- Conversations can be deleted after archiving and moved into or out of
  projects without orphaning their transcripts. `/copy` copies the latest
  assistant response, while `/export` saves Markdown through Finder or copies
  it to the clipboard.
- Desktop dictation supports configurable toggle and hold-to-talk shortcuts,
  and contextual titles are generated by Apple's on-device model after the
  first response without occupying the remote inference slot or overwriting a
  manual title.
- The final adversarial review found and closed three regressions: stale Usage
  query scope in the embedded controller, nondeterministic multi-image actions,
  and global search failing as a whole when one project disappeared.

- Runs remain attached to the conversation that created them. Logical agents
  still keep isolated rollout files for recovery and compaction, but those
  files are marked internal and never appear as chats, including in Archived
  chats. The inline Run strip now shares the composer's width, stays collapsed
  by default, limits expanded task height, and always offers Show less; the
  global Runs page uses the same readable content width as other pages.
- `/copy` copies the most recent assistant response. `/export` opens exactly
  two choices: save the complete conversation as Markdown through the native
  Finder save panel, or copy that Markdown to the clipboard. Browser clients
  use a normal file download for the first choice.
- After the first completed response, the desktop can replace the prompt-derived
  chat name with a short contextual title generated by Apple's on-device model.
  Conversation text travels over stdin to a bundled helper, never to the
  inference host, and a manually chosen title always wins.
- The sidebar's unexplained update/download icon is gone. Its phone control now
  shows the tailnet URL and a masked access token; copying the token happens in
  Electron's main process, so the secret never enters renderer state or the DOM.
