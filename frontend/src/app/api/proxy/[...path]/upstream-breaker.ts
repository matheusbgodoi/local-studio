...
export function upstreamVerdict(url: string): BreakerVerdict {
  ...
  if (Date.now() - state.downSince < COOLDOWN_MS) return state.probing ? "open" : "closed";
  if (!state.probing) {
    state.probing = true;
    return "probe";
  }
...
export function markUpstreamDown(url: string): void {
  const existing = breakers.get(url);
  if (existing) {
    existing.downSince = Date.now();
    existing.probing = true;
    return;
  }
  breakers.set(url, { downSince: Date.now(), probing: true });
}
